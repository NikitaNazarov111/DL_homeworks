...
class LanguageModel(nn.Module):
    ...
    def forward(self, indices: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        ...
        recur_output, _ = self.rnn(embeds)
        logits = self.linear(recur_output)
        return logits
    ...
    def inference(self, prefix: str = '', temp: float = 1.) -> str:
        ...
        input_indices = torch.tensor([tokens]).to(device)
        embeds = self.embedding(input_indices)    
        recur_output, hidden_state = self.rnn(embeds)
        ...
        while len(tokens) < self.max_length:
            ...
            curr_input = torch.tensor([[next_token]]).to(device)
            embeds = self.embedding(curr_input)
            
            recur_output, hidden_state = self.rnn(embeds, hidden_state)
            logits = self.linear(recur_output)
            logits = logits[:, -1, :] / temp
            probas = torch.softmax(logits, dim=-1)
