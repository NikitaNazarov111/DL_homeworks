...
class TextDataset(Dataset):
    ...
    def __init__(self, data_file: str, train: bool = True, sp_model_prefix: str = None,
        ...
        val_size = int(self.VAL_RATIO * len(texts))
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, int]:
        ...
        ind = ind[:self.max_length - 2]
        
        ind = [self.bos_id] + ind + [self.eos_id]
        length = len(ind)
        if length < self.max_length:
            ind.extend([self.pad_id] * (self.max_length - length))
        
        return torch.tensor(ind), length
