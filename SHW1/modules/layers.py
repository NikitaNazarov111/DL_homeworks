...
class BatchNormalization(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        if self.training:
            ...
            norm_eval_input = (input - self.running_mean) / np.sqrt(self.running_var + self.eps)
            if self.affine:
                y = norm_eval_input * self.weight + self.bias
            else:
                y = norm_eval_input
            return y
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        if self.training:
            if self.affine:
                grad_cap = grad_output * self.weight
            else:
                grad_cap = grad_output
            dvar = np.sum(grad_cap * self.input_mean * (- 1 / 2) * (self.inv_sqrt_var ** 3), axis=0)
            B = input.shape[0]
            dmean = np.sum(grad_cap * (- self.inv_sqrt_var), axis=0) + dvar * np.sum(- 2 * self.input_mean / B, axis=0)
            grad_input = grad_cap * self.inv_sqrt_var + dvar * 2 * self.input_mean / B + dmean / B

        else:
            if self.affine:
                grad_input = grad_output * self.weight / np.sqrt(self.running_var + self.eps)
            else:
    ...
    def update_grad_parameters(self, input: np.ndarray, grad_output: np.ndarray):
        ...
        if self.training:
            ...
            norm_eval_input = (input - self.running_mean) / np.sqrt(self.running_var + self.eps)
            self.grad_weight += np.sum(grad_output * norm_eval_input, axis=0)
            self.grad_bias += np.sum(grad_output, axis=0)
...
class Sequential(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        for module in self.modules:
            output = module.compute_output(inputs[-1])
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        # replace with your code ｀、ヽ｀、ヽ(ノ＞＜)ノ ヽ｀☂｀、ヽ
        grad_input = grad_output
        for module, inpt in zip(self.modules[::-1], self.inputs[:-1][::-1]):
            grad_input = module.backward(inpt, grad_input)
        return grad_input
